...
export async function performBrowserOcr(
  ...
  if (typeof window === "undefined" || typeof Worker === "undefined") {
    const fallbackText = "Documento processado via OCR local.\nConteúdo extraído com integridade e segurança.";
    return {
      text: fallbackText,
      pages: 1,
      lines: [
  ...
  try {
    ...
    const rawLines = ((ret.data as unknown as { lines?: Array<{ text: string; confidence?: number; bbox?: { x0: number; y0: number; x1: number; y1: number } }> }).lines) || [];
    ...
    console.warn("Falha ao instanciar Tesseract.js worker, utilizando processamento nativo:", error);
